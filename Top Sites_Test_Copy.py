...
#top_site_average_all["daily mean"] = ""
...
top_site_average_all = top_site_average_all[['Site', 'location', 'Date', 'PM2.5', 'daily max', 'daily mean', 'PM10',
    ...
    # #top_site_average_all.groupby(['Site', 'Date'])#.mean()
    # top_site_average_all = top_site_average_all[['Site', 'Date', 'PM2.5']].groupby(['Site', 'Date']).mean()
    # top_site_average_all = top_site_average_all.sort_values(by=['Site', 'Date'], ascending=True)
...
df_aq_data_hist_daily_max_loc = df_aq_data_hist_daily_max.copy()
df_aq_data_hist_daily_max_loc = df_aq_data_hist_daily_max_loc.sort_values(by=['Site', 'Date'], ascending=True)
df_aq_data_hist_daily_max_loc = df_aq_data_hist_daily_max_loc.drop(columns=['Unnamed: 0'])
#df_aq_data_hist_daily_max_loc['location'] = df_aq_data_hist_daily_max_loc['Site'].map(top_site_location)
df_aq_data_hist_daily_max_loc.to_csv('top_site_daily_max.csv')
print(df_aq_data_hist_daily_max_loc.info())
print(df_aq_data_hist_daily_max_loc)
