...
df["median_sale_price"] = df["daily max"].astype('float')
df["homes_sold"] = df["PM2.5"].astype('float')
...
for index, row in df.iterrows():
    hover_text.append(('<b>{city}</b><br><br>' +
                       ...
                       'Max PM2.5 Level: {homes_sold}<br>' +
                       'Median PM2.5 Level: {median_sale_price}<br>'
                       ).format(
                       city=row['city'],
                       Date=row['Date'],
...
for i, city in enumerate(cities, start=0):
    ...
    fig.update_layout(hoverlabel_bgcolor='#DAEEED',  # Change the background color of the tooltip to light gray
                      ...
                      yaxis_title='Max PM2.5 Level',
                      yaxis2_title='Median PM2.5 Level')
